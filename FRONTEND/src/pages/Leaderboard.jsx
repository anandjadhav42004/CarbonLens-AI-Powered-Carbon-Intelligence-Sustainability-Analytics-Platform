...
const fallbackRankings = [
  { rank: 1, name: 'Elias Vance', organization: 'Apex Conservation', streak: '365d', icon: 'workspace_premium', impact: '28.5k', avatar: 'https://lh3.googleusercontent.com/aida-public/AB6AXuCz-RyG3IzGBfiRDTwR2jfVC_LwkoRQ2lw3fuqb-Ga1tQsaLFAc2ThFio0dhnToAnZY_4AbWpsMcCnX4oO3V_yBAX1Hikn54FY_9FBzv2q3PKdKtbyICEqOcMDBM6o4c7aNEIOrPgI8qbSji51RaBLgMzXcx2Od1jzMMGIRTUbtgCke2SQWfDec33JgErxnWPQQ_tugVSTZOK_2eaaQfuWq3f0ummnDNWtHUBNVHlmkDMDwb7hQWCVegKCLg8dTmwahziSxIeeifk' },
  { rank: 2, name: 'Dr. Aris Thorne', organization: 'Boreal Initiative', streak: '90d', icon: 'verified_user', impact: '14.2k', avatar: 'https://lh3.googleusercontent.com/aida-public/AB6AXuDhmHw903vGWa8ZhWKgHn5QYme-F3cQyBIYpOFRNaJkXUVoEcJB1vrGTuE2x3GQ-tl8rk1TNGEc8EWlBln2ahK0O6c5XQhFGs1Fl-NSahxWfCDd-1l5U_9JPdJ7fJ4Nhyp0p6ihqp9fuuf4AxLX3X8YWipLEwdlMZYPtajZV8bkne7zbH7uNWOYbyzr6al8tittP3tPQhUfKarc5H76FQJAux51uj1bC2fL0iMdMjAof17XjlPneXRmmJi377VdBScNI2JsckXDX_g' },
  { rank: 3, name: 'Maya Lin', organization: 'Urban Canopy', streak: '45d', icon: 'park', impact: '11.8k', avatar: 'https://lh3.googleusercontent.com/aida-public/AB6AXuDUTlgzxZhrwUUkKJuB746FmGPlFETp8OWc-8z4OZMUrIrpwUpSl9h585IdgiDtl61XkmfPEohGiOKNwYySBIPpQSR0odqwd9Okx1kI9cnK9iwszOkk1SQQdac_1L8Qa7Omh-ABgI56QG96AV3z3jSsFI6V5wTJinLcUh1fWdJHBLeIEBLenCvW8F_-QI_mWdfwY9vWu3FlUT72RHjCzdShyX6NcpL9eDTfZ1HHzT9h16EghuRjhMs_qKm27mwK1PBuMJapCL-yVTY' },
...
const Avatar = ({ entry, className }) => (
  entry.avatar?.length === 1 ? (
    <span className="font-mono font-bold text-primary">{entry.avatar}</span>
  ) : (
    <img alt={entry.name} className={className} src={entry.avatar} />
  )
);
...
const Leaderboard = () => {
  ...
  const rankings = snapshot?.leaderboard?.length ? snapshot.leaderboard : fallbackRankings;
  const [topEntry, secondEntry, thirdEntry] = rankings;

  // Filtering standard rankings dynamically
  const filteredRankings = rankings.slice(3).filter(entry => 
    entry.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
    entry.organization.toLowerCase().includes(searchQuery.toLowerCase())
  ).filter((entry) => {
    if (activeFilter === 'Students') return entry.organization.toLowerCase().includes('research') || entry.organization.toLowerCase().includes('student');
    if (activeFilter === 'Organizations') return !entry.organization.toLowerCase().includes('research');
    return true;
  });
  const visibleRankings = filteredRankings.slice(0, visibleCount);
    ...
    <div className="flex-1 flex flex-col gap-8 text-left relative z-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-outline-variant/30 pb-6">
        <div>
          ...
          <p className="text-secondary text-sm mt-1">
            Global ranking of environmental curators determined by verified carbon sequestration metrics.
        ...
        <div className="w-full md:w-auto flex flex-col sm:flex-row gap-4">
          ...
          {filterOpen && (
            <div className="absolute right-0 top-12 bg-white border border-outline-variant rounded-2xl shadow-soft p-2 z-20 w-40">
              {['Weekly', 'Monthly', 'Global', 'Students', 'Organizations'].map((filter) => (
                <button
                  key={filter}
                  onClick={() => {
                  ...
                  className="w-full text-left px-3 py-2 rounded-xl hover:bg-surface-container-low font-mono text-[10px] uppercase text-secondary"
                >
                  {filter}
                </button>
              ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-end mb-4">
        ...
        <div className="bg-white rounded-[24px] border-2 silver-border p-6 flex flex-col items-center text-center relative overflow-hidden order-2 md:order-1 mt-0 md:mt-8 shadow-soft">
          <div className="absolute top-4 left-4 font-mono text-xs font-bold text-outline">#{secondEntry.rank}</div>
          <div className="w-20 h-20 rounded-full border-4 silver-border overflow-hidden mb-4 relative shadow-sm">
            <Avatar entry={secondEntry} className="w-full h-full object-cover" />
          </div>
          <h3 className="font-literata text-xl font-bold text-primary mb-1">{secondEntry.name}</h3>
          <p className="font-mono text-[9px] text-secondary mb-4 uppercase tracking-wider">{secondEntry.organization}</p>
          <div className="flex items-center gap-2 mb-4 bg-surface-container-low px-4 py-1.5 rounded-full inset-shadow-input">
            ...
            <span className="font-mono text-sm font-bold text-on-surface">{secondEntry.impact}</span>
        ...
        <div className="bg-white rounded-[24px] border-2 gold-border p-8 flex flex-col items-center text-center relative overflow-hidden order-1 md:order-2 transform md:-translate-y-4 shadow-luxury">
          ...
          <div className="absolute top-6 left-6 font-mono text-sm font-bold text-[#C8A97E]">#{topEntry.rank}</div>
          ...
          <div className="w-24 h-24 rounded-full border-4 gold-border overflow-hidden mb-4 relative shadow-md">
            <Avatar entry={topEntry} className="w-full h-full object-cover" />
          </div>
          <h3 className="font-literata text-2xl font-bold text-primary mb-1">{topEntry.name}</h3>
          <p className="font-mono text-[9px] text-secondary mb-5 uppercase tracking-wider">{topEntry.organization}</p>
          <div className="flex flex-col items-center gap-1 mb-5 bg-surface-container-low px-6 py-2.5 rounded-2xl inset-shadow-input w-full">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-mono text-base font-bold text-primary">{topEntry.impact}</span>
        ...
        <div className="bg-white rounded-[24px] border-2 bronze-border p-6 flex flex-col items-center text-center relative overflow-hidden order-3 mt-0 md:mt-12 shadow-soft">
          <div className="absolute top-4 right-4 font-mono text-xs font-bold text-outline">#{thirdEntry.rank}</div>
          <div className="w-16 h-16 rounded-full border-4 bronze-border overflow-hidden mb-4 relative shadow-sm">
            <Avatar entry={thirdEntry} className="w-full h-full object-cover" />
          </div>
          <h3 className="font-literata text-xl font-bold text-primary mb-1">{thirdEntry.name}</h3>
          <p className="font-mono text-[9px] text-secondary mb-4 uppercase tracking-wider">{thirdEntry.organization}</p>
          <div className="flex items-center gap-2 mb-4 bg-surface-container-low px-4 py-1.5 rounded-full inset-shadow-input">
            <span className="material-symbols-outlined text-primary text-sm">eco</span>
            <span className="font-mono text-sm font-bold text-on-surface">{thirdEntry.impact}</span>
            <span className="font-mono text-[9px] text-outline uppercase tracking-wider">tons CO₂e</span>
          </div>
          <div className="flex gap-2">
            <div className="flex items-center gap-1 bg-surface-container px-2.5 py-1 rounded-lg text-[9px] font-mono uppercase text-secondary font-bold">
        ...
        <div className="p-4 text-center bg-surface-container-low/30 border-t border-outline-variant/20">
          <button 
            onClick={() => {
              ...
              toast.success('Loaded more ledger rankings.');
            }}
          ...
          >
            {visibleCount >= filteredRankings.length ? 'All Entries Loaded' : 'Load More Entries'}
